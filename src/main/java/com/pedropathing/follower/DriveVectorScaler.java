...
public class DriveVectorScaler {
     ...
     */
    public DriveVectorScaler(Vector frontLeftVector, FollowerConstants constants) {
        ...
        mecanumVectors = new Vector[]{
                ...
                new Vector(copiedFrontLeftVector.getMagnitude(),
     ...
     *                        translational power Vector to correct onto the Bezier curve the
     *                        Follower
     *                        is following.
     ...
     *                        direction.
     * @param pathingPower    this Vector points in the direction the robot needs to go to
     *                        continue along
     *                        the Path.
     * @param robotHeading    this is the current heading of the robot, which is used to
     *                        calculate how
    ...
    public double[] getDrivePowers(Vector correctivePower,
        ...
        if (correctivePower.getMagnitude() > constants.maxPower)
            correctivePower.setMagnitude(constants.maxPower);
        if (headingPower.getMagnitude() > constants.maxPower)
            headingPower.setMagnitude(constants.maxPower);
        if (pathingPower.getMagnitude() > constants.maxPower)
            pathingPower.setMagnitude(constants.maxPower);
        ...
        if (correctivePower.getMagnitude() == constants.maxPower) {
            // checks for corrective power equal to max power scaling in magnitude. if equal,
            // then set pathing power to that
            ...
            if (leftSideVector.getMagnitude() > constants.maxPower || rightSideVector.getMagnitude() > constants.maxPower) {
                //if the combined corrective and heading power is greater than 1, then scale down
                // heading power
                double headingScalingFactor = Math.min(findNormalizingScaling(correctivePower,
                                headingPower),
                        findNormalizingScaling(correctivePower,
                                MathFunctions.scalarMultiplyVector(headingPower, -1)));
                truePathingVectors[0] = MathFunctions.subtractVectors(correctivePower,
                        MathFunctions.scalarMultiplyVector(headingPower, headingScalingFactor));
                truePathingVectors[1] = MathFunctions.addVectors(correctivePower,
                        MathFunctions.scalarMultiplyVector(headingPower, headingScalingFactor));
            } else {
                // if we're here then we can add on some drive power but scaled down to 1
                Vector leftSideVectorWithPathing = MathFunctions.addVectors(leftSideVector,
                        pathingPower);
                Vector rightSideVectorWithPathing = MathFunctions.addVectors(rightSideVector,
                        pathingPower);

                if (leftSideVectorWithPathing.getMagnitude() > constants.maxPower || rightSideVectorWithPathing.getMagnitude() > constants.maxPower) {
                    // too much power now, so we scale down the pathing vector
                    double pathingScalingFactor = Math.min(findNormalizingScaling(leftSideVector,
                            pathingPower), findNormalizingScaling(rightSideVector, pathingPower));
                    truePathingVectors[0] = MathFunctions.addVectors(leftSideVector,
                            MathFunctions.scalarMultiplyVector(pathingPower, pathingScalingFactor));
                    truePathingVectors[1] = MathFunctions.addVectors(rightSideVector,
                            MathFunctions.scalarMultiplyVector(pathingPower, pathingScalingFactor));
                } else {
        ...
        wheelPowers[0] =
                (mecanumVectorsCopy[1].getXComponent() * truePathingVectors[0].getYComponent() - truePathingVectors[0].getXComponent() * mecanumVectorsCopy[1].getYComponent()) / (mecanumVectorsCopy[1].getXComponent() * mecanumVectorsCopy[0].getYComponent() - mecanumVectorsCopy[0].getXComponent() * mecanumVectorsCopy[1].getYComponent());
        wheelPowers[1] =
                (mecanumVectorsCopy[0].getXComponent() * truePathingVectors[0].getYComponent() - truePathingVectors[0].getXComponent() * mecanumVectorsCopy[0].getYComponent()) / (mecanumVectorsCopy[0].getXComponent() * mecanumVectorsCopy[1].getYComponent() - mecanumVectorsCopy[1].getXComponent() * mecanumVectorsCopy[0].getYComponent());
        wheelPowers[2] =
                (mecanumVectorsCopy[3].getXComponent() * truePathingVectors[1].getYComponent() - truePathingVectors[1].getXComponent() * mecanumVectorsCopy[3].getYComponent()) / (mecanumVectorsCopy[3].getXComponent() * mecanumVectorsCopy[2].getYComponent() - mecanumVectorsCopy[2].getXComponent() * mecanumVectorsCopy[3].getYComponent());
        wheelPowers[3] =
                (mecanumVectorsCopy[2].getXComponent() * truePathingVectors[1].getYComponent() - truePathingVectors[1].getXComponent() * mecanumVectorsCopy[2].getYComponent()) / (mecanumVectorsCopy[2].getXComponent() * mecanumVectorsCopy[3].getYComponent() - mecanumVectorsCopy[3].getXComponent() * mecanumVectorsCopy[2].getYComponent());

        double wheelPowerMax = Math.max(Math.max(Math.abs(wheelPowers[0]),
                        Math.abs(wheelPowers[1])),
                Math.max(Math.abs(wheelPowers[2]), Math.abs(wheelPowers[3])));
        if (wheelPowerMax > constants.maxPower) {
            wheelPowers[0] = (wheelPowers[0] / wheelPowerMax) * constants.maxPower;
            wheelPowers[1] = (wheelPowers[1] / wheelPowerMax) * constants.maxPower;
            wheelPowers[2] = (wheelPowers[2] / wheelPowerMax) * constants.maxPower;
            wheelPowers[3] = (wheelPowers[3] / wheelPowerMax) * constants.maxPower;
        }
     ...
     * this one in this class, so there will be errors if you input Vectors of length greater
     * than constants.maxPower,
     * and it will scale up the variable Vector if the magnitude of the sum of the two input Vectors
     * isn't greater than constants.maxPower. So, just don't use this elsewhere. There's gotta be
     * a better way to do
     ...
     *                       magnitude of constants.maxPower.
    ...
    public double findNormalizingScaling(Vector staticVector, Vector variableVector) {
        double a = Math.pow(variableVector.getXComponent(),
                2) + Math.pow(variableVector.getYComponent(), 2);
        double b =
                staticVector.getXComponent() * variableVector.getXComponent() + staticVector.getYComponent() * variableVector.getYComponent();
        double c = Math.pow(staticVector.getXComponent(),
                2) + Math.pow(staticVector.getYComponent(), 2) - Math.pow(constants.maxPower, 2);
    ...
    public void setMaxPowerScaling(double maxPowerScaling) {
        this.constants.maxPower = MathFunctions.clamp(maxPowerScaling, 0, 1);
    }
